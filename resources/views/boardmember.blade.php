    ...
    <div class="flex flex-col w-full h-full gap-4 p-6 overflow-x-hidden overflow-y-auto">
        <section class="flex flex-col">
            <div class="flex items-center gap-2"> <a href="javascript:void(0);" onclick="location.replace(document.referrer);"
                ...
                <x-fas-circle-left class="w-4 h-4" /> </a> <h2 class="text-4xl font-extrabold">Board Member</h2>
            </div>
            <div class="overflow-hidden rounded-xl">
                {{-- <header class="relative w-full  h-24">
                    ...
                    <div class="flex flex-col  align-middle mt-14">
                        ...
                        <div
                    class="flex flex-col flex-grow w-full gap-4 p-4 overflow-x-hidden overflow-y-auto rounded-xl">
                    <div class="flex items-center gap-4">
                        <x-avatar name="{{ $owner->name }}" asset="{{ $owner->image_path }}"
                            class="!flex-shrink-0 !flex-grow-0 w-12" />
                        <p class="flex-grow truncate ps-2">{{ $owner->name }}</p>
                    ...
                    @foreach ($members as $member)
                        <div class="flex items-center gap-4">
                            <x-avatar name="{{ $member->name }}" asset="{{ $member->image_path }}"
                                class="!flex-shrink-0 !flex-grow-0 w-12" />
                            <p class="w-40 truncate font-medium ps-2">{{ $member->name }}</p>
                        </div>
                    @endforeach
                </div>
